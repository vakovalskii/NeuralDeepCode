# Remove `packages/ndcode/src/storage/db.ts`

## Goal

Remove all production usages of the legacy `packages/ndcode/src/storage/db.ts` module.
...
This does not mean removing SQLite or Drizzle everywhere in one step. The smaller target is deleting the ndcode legacy wrapper by moving call sites onto deeper modules or onto the core/effect database adapter directly.

## Current Inventory

Production imports from `packages/ndcode/src/storage/db.ts` are concentrated in 22 source files:

- `packages/ndcode/src/account/repo.ts`
- `packages/ndcode/src/cli/cmd/db.ts`
- `packages/ndcode/src/cli/cmd/import.ts`
- `packages/ndcode/src/cli/cmd/stats.ts`
- `packages/ndcode/src/control-plane/workspace.ts`
- `packages/ndcode/src/index.ts`
- `packages/ndcode/src/node.ts`
- `packages/ndcode/src/permission/index.ts`
- `packages/ndcode/src/project/project.ts`
- `packages/ndcode/src/server/projectors.ts`
- `packages/ndcode/src/server/routes/instance/httpapi/handlers/sync.ts`
- `packages/ndcode/src/server/shared/fence.ts`
- `packages/ndcode/src/session/message-v2.ts`
- `packages/ndcode/src/session/projectors.ts`
- `packages/ndcode/src/session/prompt.ts`
- `packages/ndcode/src/session/session.ts`
- `packages/ndcode/src/session/todo.ts`
- `packages/ndcode/src/share/share-next.ts`
- `packages/ndcode/src/storage/db.ts`
- `packages/ndcode/src/sync/index.ts`
- `packages/ndcode/src/worktree/index.ts`
...
Status: Completed. Startup, the public node export, and database CLI tooling no longer import the legacy ndcode database wrapper; `packages/ndcode/src/storage/db.ts` has been deleted.
...
- Move database path and client startup behind the core/effect database module rather than the ndcode wrapper.
...
Status: Completed. `SyncEvent` and the ndcode projector boundary were removed; session/message event projection now lives in core EventV2/projector infrastructure.
...
Status: Completed. These services no longer import the legacy ndcode database wrapper.
...
Status: Completed. Session/message reads and projector writes have moved off the legacy ndcode database wrapper.
...
Status: Completed. Remaining one-off CLI/admin reads and writes now use core database services or domain services instead of the legacy ndcode database wrapper.
...
All migration groups are complete or superseded. `packages/ndcode/src/storage/db.ts` has been deleted.

## Superseded: Data Migrations

Status: Superseded. No ndcode data-migration group remains.

The previous ndcode `data-migration.ts` service only backfilled session usage from message rows. That work is now covered by core database migration `packages/core/src/database/migration/20260510033149_session_usage.ts`, so there is no separate ndcode data-migration group.
...
- Existing schema ownership remains in `packages/core/src/**/*.sql.ts`; do not move table definitions back into `packages/ndcode`.

## Verification Commands

- `rg "@/storage/db|./storage/db|Database\.(use|transaction|effect|Client|getPath)|\bTxOrDb\b|\bTransaction\b" packages/ndcode/src`
- `bun typecheck` from `packages/ndcode`
- Relevant package tests from `packages/ndcode`, not the repo root
