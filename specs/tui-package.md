...
Move the canonical NeuralDeepCode terminal application from
`packages/ndcode/src/cli/cmd/tui` into a self-contained workspace package while
...
```text
packages/ndcode ---\
                      > @neuraldeepcode/tui -> @neuraldeepcode/sdk
packages/cli --------/
```
...
generic presentation libraries. It must not depend on `packages/ndcode`,
`packages/cli`, or `@neuraldeepcode/core`.

The SDK is the TUI's NeuralDeepCode boundary. Missing backend data or operations must
...
- Do not preserve private imports by creating aliases from `packages/tui` back
  into `packages/ndcode`.
- Do not replace private `packages/ndcode` imports with `@neuraldeepcode/core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@neuraldeepcode/tui`.

## Ownership Boundary

### `@neuraldeepcode/tui` Owns
...
- NeuralDeepCode domain data displayed by the TUI
...
```text
packages/ndcode/src/cli/cmd/tui
```

Its private dependency on `packages/ndcode` is primarily expressed through
the `@/*` TypeScript alias, which resolves to `packages/ndcode/src/*`.
...
Status: Completed. The private `@neuraldeepcode/tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@neuraldeepcode/tui`.
...
- The package has no dependency on `ndcode`, `@neuraldeepcode/cli`, or
  `@neuraldeepcode/core`.
...
primitives now live in `@neuraldeepcode/tui`. The legacy host consumes explicit
...
and build inputs through `@neuraldeepcode/tui/runtime`. Movable app, component,
route, and feature-plugin code no longer reads NeuralDeepCode globals or process state;
...
Replace process-global NeuralDeepCode state with resolved TUI inputs.
...
`@neuraldeepcode/tui/config` now owns schemas, defaults, keybind resolution, the
...
- Keep config path discovery, project/global precedence, migration, variable
  expansion, and plugin package installation in `packages/ndcode` initially.
...
canonical in `@neuraldeepcode/tui`. Configured references resolve through the new
...
adapters rather than NeuralDeepCode domain implementations.

Make the SDK the only NeuralDeepCode domain boundary used by the TUI.
...
- Domain-facing TUI code imports NeuralDeepCode data and operations only from
  `@neuraldeepcode/sdk`.
...
`@neuraldeepcode/tui`. The legacy host injects a narrow plugin host that retains
...
- Move plugin rendering tests to `packages/tui`; retain installation/loading
  integration tests in `packages/ndcode`.
...
- The package has no imports from `packages/ndcode`, `packages/cli`, or
  `@neuraldeepcode/core`.
...
public `@neuraldeepcode/tui` root while retaining worker/server/config/plugin and
...
- Keep the legacy yargs commands corresponding to current `thread.ts` and
  `attach.ts` in `packages/ndcode`.
- Keep the legacy embedded worker and server startup in `packages/ndcode`.
- Change those adapters to load config, create transport inputs, and call the
  public `@neuraldeepcode/tui` API.
...
- Remove obsolete `@tui/*` path mappings from `packages/ndcode`.
- Remove stale test fixtures and update all imports to package exports.
- Narrow `@neuraldeepcode/tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@neuraldeepcode/core`, or either
  executable package.
...
- TUI package imports do not reach into executable or backend implementation
  packages.
- SDK wire data is treated as the source of truth for NeuralDeepCode domain state.
...
cd packages/ndcode && bun typecheck
...
rg '@neuraldeepcode/core|packages/ndcode|packages/cli' packages/tui
rg 'src/cli/cmd/tui|@tui/' packages/ndcode/src packages/ndcode/test
```
...
- Build the current-platform `packages/ndcode` binary.
