...
The ndcode server has moved to the Effect HttpApi backend. Remaining work is
...
- remove the public in-memory `@neuraldeepcode/llm` tool loop after replacing its
...
- plugins get global "ndcode" instance like in that post i showed
- ndcode instance has stuff like `ndcode.session.prompt()` or
  `ndcode.tool.register({...})`
...
- decide whether to preserve deprecated `@neuraldeepcode/llm` orchestration exports
