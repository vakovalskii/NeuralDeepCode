...
Both `action` and `resource` use ndcode's existing wildcard matching behavior.
...
The relative policy precedence of direct project files and `.ndcode` files is intentionally deferred until `.ndcode` configuration is reviewed.
...
Plugins must not be allowed to add, remove, or override policy statements. Plugins can contribute functionality or configured providers; policy determines whether ndcode permits an operation through its managed execution paths.
