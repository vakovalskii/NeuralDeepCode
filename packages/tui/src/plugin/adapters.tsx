import type { TuiDialogSelectOption, TuiPluginApi, TuiSlotProps } from "@neuraldeepcode/plugin/tui"
...
import type { useNeuralDeepCodeKeymap } from "../keymap"
...
type Input = {
  ...
  keymap: ReturnType<typeof useNeuralDeepCodeKeymap>
...
export function createTuiApiAdapters(input: Input): Omit<TuiPluginApi, "lifecycle"> {
  return {
    ...
    mode: {
      current() {
        return Keymap.getNeuralDeepCodeModeStack(input.keymap).current()
      },
      push(mode) {
        return Keymap.getNeuralDeepCodeModeStack(input.keymap).push(mode)
      },
    },
    route: {
      register(list) {
