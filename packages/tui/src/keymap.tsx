...
export const NDC_BASE_MODE = "base"
export const COMMAND_PALETTE_COMMAND = "command.palette.show"

const NDC_MODE_KEY = "ndcode.mode"

export const NeuralDeepCodeKeymapProvider = KeymapProvider
export const useNeuralDeepCodeKeymap = useKeymap

export { useBindings, useKeymapSelector }

export type OpenTuiKeymap = ReturnType<typeof useKeymap>
type NeuralDeepCodeModeStack = ReturnType<typeof createNeuralDeepCodeModeStack>
...
const modeStacks = new WeakMap<OpenTuiKeymap, NeuralDeepCodeModeStack>()

function isVisiblePaletteCommand(command: Command) {
  return command.hidden !== true && command.name !== COMMAND_PALETTE_COMMAND
}

export function createNeuralDeepCodeModeStack(keymap: OpenTuiKeymap) {
  keymap.setData(NDC_MODE_KEY, NDC_BASE_MODE)

  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(NDC_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(NDC_MODE_KEY, stack.at(-1)?.mode ?? NDC_BASE_MODE)
  }

  const stackApi = {
    current() {
      return stack.at(-1)?.mode ?? NDC_BASE_MODE
    },
    push(mode: string) {
      if (disposed) return () => {}
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(NDC_MODE_KEY, undefined)
...
export function useNeuralDeepCodeModeStack() {
  return getNeuralDeepCodeModeStack(useNeuralDeepCodeKeymap())
}

export function getNeuralDeepCodeModeStack(keymap: OpenTuiKeymap) {
  const value = modeStacks.get(keymap)
  if (!value) throw new Error("NeuralDeepCode mode stack is not registered for this keymap")
  return value
}
...
export function registerNeuralDeepCodeKeymap(keymap: OpenTuiKeymap, renderer: CliRenderer, config: ResolvedKeymapConfig) {
  const modeStack = createNeuralDeepCodeModeStack(keymap)
...
export function useCommandSlashes(): Accessor<readonly CommandSlashEntry[]> {
  const keymap = useNeuralDeepCodeKeymap()
