...
import {
  ...
  NDC_BASE_MODE,
  NeuralDeepCodeKeymapProvider,
  registerNeuralDeepCodeKeymap,
  useBindings,
  useNeuralDeepCodeKeymap,
} from "./keymap"
  ...
  const result = yield* Effect.scoped(
    Effect.gen(function* () {
      const renderer = yield* Effect.acquireRelease(
        Effect.tryPromise(() =>
          createCliRenderer({
            ...
            useMouse: !Flag.NDC_DISABLE_MOUSE && input.config.mouse,
            consoleOptions: {
      ...
      yield* Effect.acquireRelease(
        Effect.sync(() => registerNeuralDeepCodeKeymap(keymap, renderer, input.config)),
                ...
                <ErrorBoundary fallback={(error, reset) => <ErrorComponent error={error} reset={reset} mode={mode} />}>
                  ...
                  >
                    ...
                    >
                      <TuiStartupProvider
                        value={{
                          initialRoute: process.env.NDC_ROUTE ? JSON.parse(process.env.NDC_ROUTE) : undefined,
                          skipInitialLoading: Boolean(process.env.NDC_FAST_BOOT),
                        }}
                      >
                        <ClipboardProvider>
                          <NeuralDeepCodeKeymapProvider keymap={keymap}>
...
function App(props: { onSnapshot?: () => Promise<string[]>; pluginHost: TuiPluginHost }) {
  ...
  const offSelectionKeys = keymap.intercept(
    "key",
    ({ event }) => {
      if (!Flag.NDC_EXPERIMENTAL_DISABLE_COPY_ON_SELECT) return
  ...
  createEffect(() => {
    if (!terminalTitleEnabled() || Flag.NDC_DISABLE_TERMINAL_TITLE) return

    if (route.data.type === "home") {
      renderer.setTerminalTitle("NeuralDeepCode")
      return
    }
  ...
  const appCommands = createMemo(() =>
    [
      ...
      },
      {
        ...
        category: "Workspace",
        hidden: !Flag.NDC_EXPERIMENTAL_WORKSPACES,
      ...
      },
      {
        ...
        title: "Open docs",
        run: () => {
          open("https://neuraldeep.ru/docs").catch(() => {})
          dialog.clear()
        },
        category: "System",
      },
  ...
  async function promptUpdate(version: string) {
    ...
    await DialogAlert.show(
      dialog,
      "Update Complete",
      `Successfully updated to NeuralDeepCode v${result.data.version}. Please restart the application.`,
    )
  ...
  return (
    <box
      ...
      onMouseUp={
        !Flag.NDC_EXPERIMENTAL_DISABLE_COPY_ON_SELECT
          ? () => Selection.copy(renderer, toast, clipboard)
          : undefined
      }
    >
      <Show when={Flag.NDC_SHOW_TTFD}>
        <TimeToFirstDraw />
      </Show>
      <Show when={ready()}>
