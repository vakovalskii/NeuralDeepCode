...
import { getNeuralDeepCodeModeStack, NDC_BASE_MODE, NeuralDeepCodeKeymapProvider, registerNeuralDeepCodeKeymap } from "../src/keymap"
...
test("legacy page key aliases compile as page keys", async () => {
  ...
  function Harness() {
    ...
    const offKeymap = registerNeuralDeepCodeKeymap(keymap, renderer, config)
...
test("mode-less bindings stay active when ndcode mode changes", async () => {
  ...
  function Harness() {
    ...
    const popQuestion = getNeuralDeepCodeModeStack(keymap).push("question")
    counts.question = activeCounts()
    popQuestion()
    const popAutocomplete = getNeuralDeepCodeModeStack(keymap).push("autocomplete")
