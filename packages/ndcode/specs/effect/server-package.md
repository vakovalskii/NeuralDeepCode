...
Practical reference for a future `packages/server` split after the ndcode
server moved to the Effect HttpApi backend.

## Current State

- The server still lives in `packages/ndcode`.
...
Until enough shared service code lives outside `packages/ndcode`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/ndcode`

It should not import `packages/ndcode` services while `packages/ndcode`
...
2. Move stable domain schemas into shared packages only when they no longer
   depend on ndcode-local runtime modules.
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/ndcode` implementation details.
