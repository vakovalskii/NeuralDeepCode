...
- Author package entrypoint is `@neuraldeepcode/plugin/tui`.
...
```json
{
  "$schema": "https://neuraldeep.ru/tui.json",
  "theme": "smoke-theme",
  "leader_timeout": 2000,
  "keybinds": {
    "leader": "ctrl+x",
  ...
  "plugin": ["@acme/ndcode-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  },
  "attention": {
...
- Import types from `@neuraldeepcode/plugin/tui`.
- `@neuraldeepcode/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@neuraldeepcode/plugin/tui"
...
- The value is a semver range checked against the running NeuralDeepCode version.
- If the range is not satisfied, the plugin is skipped with a warning and a session error.
- If `engines.ndcode` is absent, no check is performed (backward compatible).
...
- `ndcode plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `ndcode plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.ndcode` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.ndcode`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.ndcode`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.ndcode` dir or `NDC_CONFIG_DIR`, NeuralDeepCode installs `@neuraldeepcode/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@neuraldeepcode/plugin/tui`.
...
- The host already installs the default OpenTUI bundle (`default keys`, metadata fields, and enabled fields) plus NeuralDeepCode's comma bindings, leader token, base layout fallback, pending-sequence helpers, and managed textarea layer.
...
NeuralDeepCode registers a `mode` layer field on the host keymap. Plugins can use it to keep bindings active only in the relevant UI state.
...
Mode pushes are automatically tracked by the plugin runtime. If a plugin is disabled, fails during activation, or the TUI shuts down before the plugin calls the disposer, NeuralDeepCode pops the plugin's pushed modes during plugin cleanup. Calling the disposer yourself is still recommended for component lifetimes; cleanup remains idempotent.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@neuraldeepcode/plugin/tui`.
...
- `message` is required; `title` defaults to `"ndcode"`; `notification` defaults to enabled with `when: "blurred"`; `sound` defaults to enabled with `when: "always"`.
...
- Config `attention.sounds` overrides active-pack sounds by slot. Failed loads fall back to the active pack and then `ndcode.default`.
...
- Local plugins persist installed themes under the local `.ndcode/themes` area near the plugin config source.
...
- `--pure` / `NDC_PURE` skips external TUI plugins only.
...
- Local smoke plugin: `.ndcode/plugins/tui-smoke.tsx`
- Local vim plugin: `.ndcode/plugins/tui-vim.tsx`
- Local smoke config: `.ndcode/tui.json`
- Local smoke theme: `.ndcode/plugins/smoke-theme.json`
