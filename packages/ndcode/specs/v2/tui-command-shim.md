...
Delete `packages/ndcode/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/ndcode/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/ndcode/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/ndcode`
- TUI plugin loader tests in `packages/ndcode` if runtime plugin API wiring changed
