...
Trim `packages/ndcode/src/server/routes/instance/httpapi/public.ts` until OpenAPI generation is mostly a direct projection of the `HttpApi` route declarations, without breaking the generated SDK surface.
...
- `[x]` Add or extend `packages/ndcode/test/server/httpapi-query-schema-drift.test.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/ndcode`.
- `[x]` `bun typecheck` from `packages/ndcode`.

### PR 2: Delete Spec-Only Workspace Query Injection

- `[x]` Edit `packages/ndcode/src/server/routes/instance/httpapi/public.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/ndcode`.
- `[x]` `bun dev generate > /tmp/ndcode-openapi.json` from `packages/ndcode`.
...
- Edit `packages/ndcode/src/server/routes/instance/httpapi/public.ts`.
...
- `bun dev generate > /tmp/ndcode-openapi.json` from `packages/ndcode`.
...
- Check source schemas in files like `packages/ndcode/src/session/schema.ts`, `packages/ndcode/src/permission/schema.ts`, and pty schema definitions.
...
- Edit route group files under `packages/ndcode/src/server/routes/instance/httpapi/groups/`.
- Replace SDK-visible `HttpApiError.BadRequest` / `HttpApiError.NotFound` with explicit error schemas from `packages/ndcode/src/server/routes/instance/httpapi/errors.ts` or add new ones there.
