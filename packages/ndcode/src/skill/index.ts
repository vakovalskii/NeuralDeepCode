...
import { FrontmatterError } from "@neuraldeepcode/core/v1/config/error"
import { ConfigMarkdown } from "@/config/markdown"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Glob } from "@neuraldeepcode/core/util/glob"
...
const NDC_SKILL_PATTERN = "{skill,skills}/**/SKILL.md"
const SKILL_PATTERN = "**/SKILL.md"

// Built-in skill that ships with ndcode. The model's intuition for what an
// ndcode.json should look like is often wrong, and ndcode hard-fails on
// invalid config, so users hit cryptic startup errors. Loading this skill
// when the model is asked to touch ndcode's own config files gives it the
// actual schemas instead of guesses.
const CUSTOMIZE_NDC_SKILL_NAME = "customize-ndcode"
const CUSTOMIZE_NDC_SKILL_DESCRIPTION =
...
const CUSTOMIZE_NDC_SKILL_BODY = SkillPlugin.CustomizeNeuralDeepCodeContent
...
const discoverSkills = Effect.fnUntraced(function* (
  ...
  for (const dir of configDirs) {
    yield* scan(state, dir, NDC_SKILL_PATTERN)
  }
...
export class Service extends Context.Service<Service, Interface>()("@neuraldeepcode/Skill") {}
  ...
  Effect.gen(function* () {
    ...
    const state = yield* InstanceState.make(
      Effect.fn("Skill.state")(function* () {
        ...
        s.skills[CUSTOMIZE_NDC_SKILL_NAME] = {
          name: CUSTOMIZE_NDC_SKILL_NAME,
          description: CUSTOMIZE_NDC_SKILL_DESCRIPTION,
          location: "<built-in>",
          content: CUSTOMIZE_NDC_SKILL_BODY,
        }
