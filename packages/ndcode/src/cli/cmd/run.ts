import type { PermissionV1 } from "@neuraldeepcode/core/v1/permission"
// CLI entry point for `ndcode run`.
//
// Handles three modes:
...
//      ndcode server and runs interactive mode against it.
//
...
import { createNeuralDeepCodeClient, type NeuralDeepCodeClient, type ToolPart } from "@neuraldeepcode/sdk/v2"
import { FormatError, FormatUnknownError } from "../error"
import { INTERACTIVE_INPUT_ERROR, resolveInteractiveStdin } from "./run/runtime.stdin"

type ModelInput = Parameters<NeuralDeepCodeClient["session"]["prompt"]>[0]["model"]
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run ndcode with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running ndcode server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to NDC_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to NDC_SERVER_USERNAME or 'ndcode')",
      })
      .option("dir", {
        type: "string",
  ...
  handler: Effect.fn("Cli.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      const attachSDK = (dir?: string) => {
        return createNeuralDeepCodeClient({
          baseUrl: args.attach!,
          directory: dir,
      ...
      async function session(sdk: NeuralDeepCodeClient): Promise<SessionInfo | undefined> {
        if (args.session) {
      ...
      async function share(sdk: NeuralDeepCodeClient, sessionID: string) {
      ...
      async function current(sdk: NeuralDeepCodeClient): Promise<string> {
        if (!args.attach) {
      ...
      async function attachAgent(sdk: NeuralDeepCodeClient) {
        if (!args.agent) return undefined
        const name = args.agent
      ...
      async function pickAgent(sdk: NeuralDeepCodeClient) {
        if (!args.agent) return undefined
        if (args.attach) {
      ...
      async function execute(sdk: NeuralDeepCodeClient) {
        ...
        async function loop(client: NeuralDeepCodeClient, events: Awaited<ReturnType<typeof sdk.event.subscribe>>) {
      ...
      const sdk = createNeuralDeepCodeClient({
        baseUrl: "http://ndcode.internal",
        fetch: fetchFn,
        directory,
      })
      await execute(sdk)
