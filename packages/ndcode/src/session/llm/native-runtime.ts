...
import type { LLMClientShape } from "@neuraldeepcode/llm/route"
...
function statusWithFetch(
  ...
  if (providerID !== "openai" && providerID !== "anthropic" && !providerID.startsWith("ndcode"))
    return { type: "unsupported", reason: "provider is not openai, ndcode, or anthropic" }
...
export function stream(input: StreamInput): StreamResult {
  ...
  // Integration point with @neuraldeepcode/llm: native-request lowers session data
...
export function nativeTools(tools: Record<string, Tool>, input: Pick<StreamInput, "messages" | "abort">) {
  return Object.fromEntries(
    Object.entries(tools).map(([name, item]) => [
      name,
      // Tool execution remains ndcode-owned. The native runtime only adapts
      // the @neuraldeepcode/llm tool call back into the AI SDK Tool.execute shape.
      NativeTool.make({
