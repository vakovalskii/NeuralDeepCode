...
export const DEPRECATED_PLUGIN_PACKAGES = ["ndcode-openai-codex-auth", "ndcode-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, ndcodeVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(ndcodeVersion) || semver.major(ndcodeVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.ndcode
  if (typeof range !== "string") return
  if (!semver.satisfies(ndcodeVersion, range)) {
    throw new Error(`Plugin requires ndcode ${range} but running ${ndcodeVersion}`)
  }
}
