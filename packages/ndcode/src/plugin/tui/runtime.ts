...
import {
  ...
  type TuiSlotPlugin,
  type TuiTheme,
} from "@neuraldeepcode/plugin/tui"
import path from "path"
import { fileURLToPath } from "url"
import { TuiConfig } from "@/config/tui"
import { errorData, errorMessage } from "@neuraldeepcode/tui/util/error"
...
import { hasTheme, upsertTheme } from "@neuraldeepcode/tui/context/theme"
import { Global } from "@neuraldeepcode/core/global"
import { Filesystem } from "@/util/filesystem"
import { Process } from "@/util/process"
import { Flock } from "@neuraldeepcode/core/util/flock"
import { Flag } from "@neuraldeepcode/core/flag/flag"
import { internalTuiPlugins, type InternalTuiPlugin } from "./internal"
import type { HostPluginApi, HostSlots } from "@neuraldeepcode/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@neuraldeepcode/core/v1/config/plugin"
import { createCommandShim } from "@neuraldeepcode/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@neuraldeepcode/tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".ndcode"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".ndcode", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".ndcode", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.NDC_PURE ? [] : pluginOrigins
    if (Flag.NDC_PURE && pluginOrigins.length) {
    }
