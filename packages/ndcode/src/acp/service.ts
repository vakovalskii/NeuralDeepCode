...
import type { Message, NeuralDeepCodeClient, SessionMessageResponse } from "@neuraldeepcode/sdk/v2"
...
export const AuthMethodID = "ndcode-login"
...
export class Service extends Context.Service<Service, Interface>()("@neuraldeepcode/ACP/Service") {}
...
export function make(input: {
  ...
  const initialize = Effect.fn("ACP.initialize")(function* (params: InitializeRequest) {
    ...
    const authMethod: AuthMethod = {
      description: "Run `ndcode auth login` in the terminal",
      name: "Login with ndcode",
      id: AuthMethodID,
    }
...
function makeDirectoryService(sdk: NeuralDeepCodeClient) {
...
function makeUsageService(sdk: NeuralDeepCodeClient) {
...
async function loadDirectorySnapshot(sdk: NeuralDeepCodeClient, directory: string) {
...
function defaultModelFromConfig(
  ...
  // a default. Configured model, ndcode provider, then sorted best model keep
  // the protocol response deterministic without extra session/message reads.
  const ndcodeProvider = providers[ProviderV2.ID.make("ndcode")]
  const ndcodeModel = ndcodeProvider ? Provider.sort(Object.values(ndcodeProvider.models))[0] : undefined
  if (ndcodeProvider && ndcodeModel) return { providerID: ndcodeProvider.id, modelID: ndcodeModel.id }
...
function fromUnknownError(error: unknown, service?: string): Error {
  ...
  return new ACPError.ServiceFailureError({ safeMessage: "NeuralDeepCode service failure", service })
}
