...
import { NeuralDeepCodeHttpApi } from "./api"
...
const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for ndcode serve and web commands",
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw: OpenApiSpec = OpenApi.fromApi(NeuralDeepCodeHttpApi)
...
export const PublicApi = NeuralDeepCodeHttpApi.annotateMerge(
  OpenApi.annotations({
    title: "ndcode",
    version: "1.0.0",
    description: "ndcode api",
