This is a plugin to simulate a remote environment locally. Add this to `.ndcode/ndcode.jsonc`:

```json
  "plugin": ["../packages/ndcode/src/control-plane/dev/debug-workspace-plugin.ts"],
```

In a separate terminal, run a separate NeuralDeepCode server. This will act like a remote server and the local instance will proxy all requests to it:

```
./packages/ndcode/script/run-workspace-server
```

With the plugin install, you can now run NeuralDeepCode and create a `debug` workspace type. This will create a "remote" workspace which talks to the second workspace server started above.
