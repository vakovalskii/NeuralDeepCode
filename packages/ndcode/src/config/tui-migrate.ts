...
const TUI_SCHEMA_URL = "https://neuraldeep.ru/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from ndcode.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const ndcode = await ndcodeFiles(input)
  for (const file of ndcode) {
...
async function ndcodeFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "ndcode"),
    ...(await Filesystem.findUp(["ndcode.json", "ndcode.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "ndcode"))
  }
  if (Flag.NDC_CONFIG) files.push(Flag.NDC_CONFIG)
