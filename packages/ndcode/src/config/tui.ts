...
import { isRecord } from "@neuraldeepcode/tui/util/record"
...
import { TuiKeybind } from "@neuraldeepcode/tui/config/keybind"
...
import { TuiConfig } from "@neuraldeepcode/tui/config"
...
export class Service extends Context.Service<Service, Interface>()("@neuraldeepcode/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old ndcode.json shape) still get their settings applied.
  ...
  // Every config dir we may read from: global config dir, any `.ndcode`
  // folders between cwd and home, and NDC_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.NDC_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit NDC_TUI_CONFIG override, if set.
  if (Flag.NDC_TUI_CONFIG) {
    const configFile = Flag.NDC_TUI_CONFIG
  ...
  // 4. `.ndcode` directories (and NDC_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".ndcode") || dir === Flag.NDC_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".ndcode") && dir !== Flag.NDC_CONFIG_DIR) continue
