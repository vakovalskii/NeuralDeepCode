...
import type { HostPluginApi } from "@neuraldeepcode/tui/plugin/slots"
...
export function createTuiPluginApi(opts: Opts = {}): HostPluginApi {
  ...
  const own = createNeuralDeepCodeClient({
  ...
  let selected = opts.theme?.selected ?? "ndcode"
  const set =
    opts.theme?.set ??
    ((name: string) => {
  ...
  return {
    ...
    attention: {
      ...
      soundboard: {
        ...
        current: () => opts.attention?.soundboard?.current?.() ?? "ndcode.default",
