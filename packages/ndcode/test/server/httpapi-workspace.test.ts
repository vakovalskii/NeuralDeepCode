...
function requestServer(path: string, directory: string, init: RequestInit = {}) {
  ...
  headers.set("x-ndcode-directory", directory)
      ...
      try {
        const response = yield* requestDefault(url.toString(), dir, {
          ...
          headers: {
            "accept-encoding": "br",
            "content-type": "application/json",
            "x-ndcode-workspace": "internal",
          },
          body: JSON.stringify({ $schema: "https://neuraldeep.ru/config.json" }),
        })
        ...
        expect(forwarded[0]?.headers).not.toHaveProperty("x-ndcode-directory")
        expect(forwarded[0]?.headers).not.toHaveProperty("x-ndcode-workspace")
