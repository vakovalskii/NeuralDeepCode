...
import { EventSequenceTable } from "@neuraldeepcode/core/event/sql"
...
import { WorkspaceV2 } from "@neuraldeepcode/core/workspace"
import { WorkspaceTable } from "@neuraldeepcode/core/control-plane/workspace.sql"
...
import { Ripgrep } from "@neuraldeepcode/core/ripgrep"

const originalEnv = {
  NDC_AUTH_CONTENT: process.env.NDC_AUTH_CONTENT,
  NDC_EXPERIMENTAL_WORKSPACES: process.env.NDC_EXPERIMENTAL_WORKSPACES,
...
beforeEach(() => {
  restoreEnv()
  process.env.NDC_EXPERIMENTAL_WORKSPACES = "true"
})
...
async function initGitRepo(dir: string) {
  ...
  await $`git config user.email "test@ndcode.test"`.cwd(dir).quiet()
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const instance = yield* requireInstance
        const workspace = yield* Workspace.Service
        process.env.NDC_AUTH_CONTENT = JSON.stringify({ test: { type: "api", key: "secret" } })
        process.env.OTEL_EXPORTER_OTLP_HEADERS = "authorization=otel"
        process.env.OTEL_EXPORTER_OTLP_ENDPOINT = "https://otel.test"
        process.env.OTEL_RESOURCE_ATTRIBUTES = "service.name=ndcode-test"
        ...
        expect(JSON.parse(recorded.calls.create[0].env.NDC_AUTH_CONTENT ?? "{}")).toEqual({
          test: { type: "api", key: "secret" },
        })
        expect(recorded.calls.create[0].env.NDC_WORKSPACE_ID).toBe(workspaceID)
        expect(recorded.calls.create[0].env.NDC_EXPERIMENTAL_WORKSPACES).toBe("true")
        ...
        expect(recorded.calls.create[0].env.OTEL_RESOURCE_ATTRIBUTES).toBe("service.name=ndcode-test")
