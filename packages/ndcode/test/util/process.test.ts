...
describe("util.process", () => {
  ...
  test("merges environment overrides", async () => {
    const out = await Process.run(node('process.stdout.write(process.env.NDC_TEST ?? "")'), {
      env: {
        NDC_TEST: "set",
      },
    })
  ...
  test("uses shell in run on Windows", async () => {
    ...
    const out = await Process.run(["set", "NDC_TEST_SHELL"], {
      shell: true,
      env: {
        NDC_TEST_SHELL: "ok",
      },
    ...
    expect(out.code).toBe(0)
    expect(out.stdout.toString()).toContain("NDC_TEST_SHELL=ok")
  })
