...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "ndcode.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "ndcode.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "ndcode.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "ndcode.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["ndcode.json", "ndcode.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "ndcode.json"),
        path.join(project, "ndcode.jsonc"),
      ])
    })
  })
