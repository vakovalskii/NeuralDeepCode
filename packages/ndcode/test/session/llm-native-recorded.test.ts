...
import { SessionV1 } from "@neuraldeepcode/core/v1/session"
import { FSUtil } from "@neuraldeepcode/core/fs-util"
import { ModelsDev } from "@neuraldeepcode/core/models-dev"
import { HttpRecorder } from "@neuraldeepcode/http-recorder"
import { HttpRecorderInternal } from "@neuraldeepcode/http-recorder/internal"
...
import { LLMEvent, LLMResponse } from "@neuraldeepcode/llm"
import { LLMClient, RequestExecutor, WebSocketExecutor } from "@neuraldeepcode/llm/route"
import { Env } from "@/env"
...
function decodeRecordOpenAIOAuth() {
  const value = process.env.NDC_RECORD_OPENAI_AUTH
...
const RECORDED_SCENARIOS = [
  {
    ...
    tags: ["ndcode", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("NDC_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.openai,
        name: "OpenAI",
        ...
        model,
        options: {
          apiKey: envValue("NDC_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY") ?? "fixture-openai-key",
  ...
  },
  {
    ...
    protocol: "openai-responses",
    tags: ["ndcode", "native", "oauth", "tool-loop"],
  ...
  {
    id: "ndcode-proxy",
    name: "NeuralDeepCode proxy",
    providerID: ProviderV2.ID.ndcode,
    modelID: "gpt-5.2-codex",
    cassette: "session/native-zen-tool-loop",
    protocol: "openai-responses",
    tags: ["ndcode", "zen", "native", "tool-loop"],
    canRecord: () => Boolean(process.env.NDC_RECORD_CONSOLE_TOKEN && process.env.NDC_RECORD_ZEN_ORG_ID),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.ndcode,
        name: "NeuralDeepCode Zen",
        env: ["NDC_CONSOLE_TOKEN"],
        npm: "@ai-sdk/openai-compatible",
        api: zenURL(process.env.NDC_RECORD_ZEN_CONNECTION ?? "fixture"),
        model,
        options: {
          apiKey: process.env.NDC_RECORD_CONSOLE_TOKEN ?? "fixture-console-token",
          headers: { "x-org-id": process.env.NDC_RECORD_ZEN_ORG_ID ?? "fixture-org" },
        },
      }),
  },
  {
    ...
    tags: ["ndcode", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("NDC_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.anthropic,
        name: "Anthropic",
        ...
        model,
        options: {
          apiKey: envValue("NDC_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY") ?? "fixture-anthropic-key",
...
const selectedScenarios = new Set(
  (envValue("NDC_RECORDED_SCENARIO", "RECORDED_PROVIDER") ?? "")
    .split(",")
...
const recordError = (scenario: RecordedScenario) =>
  scenario.id === "openai-oauth"
    ? "Set NDC_RECORD_OPENAI_AUTH to an OAuth auth JSON object in the recording environment."
...
function recordedNativeLLMLayer(scenario: RecordedScenario) {
  ...
  // Only the HTTP client is recorded; RequestExecutor and the ndcode LLM stack remain real.
  const metadata = {
