...
it.instance("plan agent denies edits except .ndcode/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".ndcode/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".ndcode", "skill", "perm-skill")
...
`,
      ...
      const home = process.env.NDC_TEST_HOME
      process.env.NDC_TEST_HOME = test.directory
