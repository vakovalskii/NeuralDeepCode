...
describe("ndcode read-only commands (smoke)", () => {
  // `mcp list` reads MCP server config and pings each one. With the empty
  // NDC_CONFIG_CONTENT={} we provide, no servers should be configured
  // and the command should report that cleanly.
  cliIt.live(
    "mcp list: exits 0",
    ({ ndcode }) =>
      Effect.gen(function* () {
        const r = yield* ndcode.spawn(["mcp", "list"])
        ndcode.expectExit(r, 0, "mcp list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ ndcode }) =>
      Effect.gen(function* () {
        const r = yield* ndcode.spawn(["providers", "list"])
        ndcode.expectExit(r, 0, "providers list")
  ...
  cliIt.live(
    ...
    ({ ndcode }) =>
      Effect.gen(function* () {
        const r = yield* ndcode.spawn(["models"])
        ndcode.expectExit(r, 0, "models")
  ...
  cliIt.live(
    ...
    ({ ndcode }) =>
      Effect.gen(function* () {
        const r = yield* ndcode.spawn(["agent", "list"])
        ndcode.expectExit(r, 0, "agent list")
      }),
    60_000,
  )

  // `session list` reads the session DB. Fresh NDC_TEST_HOME means
  // empty DB. Exit 0 with no sessions.
  cliIt.live(
    "session list: exits 0",
    ({ ndcode }) =>
      Effect.gen(function* () {
        const r = yield* ndcode.spawn(["session", "list"])
        ndcode.expectExit(r, 0, "session list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ ndcode }) =>
      Effect.gen(function* () {
        const r = yield* ndcode.spawn(["stats"])
        ndcode.expectExit(r, 0, "stats")
      }),
    60_000,
  )
  ...
  // in production it'd be a path under NDC_TEST_HOME / XDG_DATA_HOME.
  // Accept either form — both prove the resolver ran without crashing.
  cliIt.live(
    "db path: exits 0 and prints a path or :memory:",
    ({ ndcode }) =>
      Effect.gen(function* () {
        const r = yield* ndcode.spawn(["db", "path"])
        ndcode.expectExit(r, 0, "db path")
