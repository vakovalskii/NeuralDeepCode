...
import { NeuralDeepCodeClient, type GlobalEvent } from "@neuraldeepcode/sdk/v2"
import { createSessionTransport } from "@/cli/cmd/run/stream.transport"
import type { FooterApi, FooterEvent, LocalReplayRow, RunFilePart, StreamCommit } from "@/cli/cmd/run/types"

type EventStream = Awaited<ReturnType<NeuralDeepCodeClient["event"]["subscribe"]>>["stream"]
type GlobalEventStream = Awaited<ReturnType<NeuralDeepCodeClient["global"]["event"]>>["stream"]
type SdkEvent = EventStream extends AsyncGenerator<infer T, unknown, unknown> ? T : never
type SessionMessage = NonNullable<Awaited<ReturnType<NeuralDeepCodeClient["session"]["messages"]>>["data"]>[number]
type SessionChild = NonNullable<Awaited<ReturnType<NeuralDeepCodeClient["session"]["children"]>>["data"]>[number]
type SessionToolPart = Extract<SessionMessage["parts"][number], { type: "tool" }>
type SessionStatusMap = NonNullable<Awaited<ReturnType<NeuralDeepCodeClient["session"]["status"]>>["data"]>
...
function ok<T>(data: T) {
  return Promise.resolve({
    data,
    error: undefined,
    request: new Request("https://ndcode.test"),
...
function sdk(
  input: {
    ...
    subscribe?: NeuralDeepCodeClient["event"]["subscribe"]
    globalEvent?: NeuralDeepCodeClient["global"]["event"]
    promptAsync?: NeuralDeepCodeClient["session"]["promptAsync"]
    status?: NeuralDeepCodeClient["session"]["status"]
    messages?: NeuralDeepCodeClient["session"]["messages"]
    children?: NeuralDeepCodeClient["session"]["children"]
    permissions?: NeuralDeepCodeClient["permission"]["list"]
    questions?: NeuralDeepCodeClient["question"]["list"]
  } = {},
) {
  const client = new NeuralDeepCodeClient()

  const subscribe: NeuralDeepCodeClient["event"]["subscribe"] = input.subscribe ?? (() => sse(input.stream ?? emptyStream()))
  const globalEvent: NeuralDeepCodeClient["global"]["event"] =
    input.globalEvent ?? (() => globalSse(input.globalStream ?? wrapGlobalStream(input.stream ?? emptyStream())))
  const promptAsync: NeuralDeepCodeClient["session"]["promptAsync"] = input.promptAsync ?? (() => ok(undefined))
  const status: NeuralDeepCodeClient["session"]["status"] = input.status ?? (() => ok({}))
  const messages: NeuralDeepCodeClient["session"]["messages"] = input.messages ?? (() => ok([]))
  const children: NeuralDeepCodeClient["session"]["children"] = input.children ?? (() => ok([]))
  const permissions: NeuralDeepCodeClient["permission"]["list"] = input.permissions ?? (() => ok([]))
  const questions: NeuralDeepCodeClient["question"]["list"] = input.questions ?? (() => ok([]))
