...
`,
  ...
  const pure = process.env.NDC_PURE
  const meta = process.env.NDC_PLUGIN_META_FILE
  process.env.NDC_PURE = "1"
  process.env.NDC_PLUGIN_META_FILE = tmp.extra.meta
  ...
  try {
    ...
    wait.mockRestore()
    if (pure === undefined) {
      delete process.env.NDC_PURE
    } else {
      process.env.NDC_PURE = pure
    }
    if (meta === undefined) {
      delete process.env.NDC_PLUGIN_META_FILE
    } else {
      process.env.NDC_PLUGIN_META_FILE = meta
    }
  }
})
