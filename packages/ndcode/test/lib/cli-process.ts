// Subprocess test harness for the ndcode CLI. Spawns the real binary against
...
//
// Configuration flows through ndcode's built-in test affordances:
//   - NDC_CONFIG_CONTENT      : provider config inline, no files to find
//   - NDC_TEST_HOME           : pins os.homedir() → tmpdir
//   - NDC_DISABLE_PROJECT_CONFIG : skip walking up for ndcode.json
//   - NDC_PURE                : skip external plugin discovery + install
//   - NDC_DISABLE_AUTOUPDATE / AUTOCOMPACT / MODELS_FETCH : no background work
// Plus HOME / XDG_* pointing at the tmpdir for belt-and-suspenders isolation.
//
// Today only `ndcode.run` is fully wired. The shape supports adding more
// builders (`ndcode.serve(opts)`, `ndcode.acp(opts)`, `ndcode.auth(...)`)
// without changing the fixture. Long-lived commands like `serve` will need a
// different return shape — see the TODO at the bottom of NeuralDeepCodeCli.
import { test, type TestOptions } from "bun:test"
import { FSUtil } from "@neuraldeepcode/core/fs-util"
import { AppProcess } from "@neuraldeepcode/core/process"
...
const ndcodeRoot = path.resolve(import.meta.dir, "../../")
const cliEntry = path.join(ndcodeRoot, "src/index.ts")
...
function isolatedEnv(home: string, configJson: string): Record<string, string> {
  return {
    ...
    NDC_CONFIG_CONTENT: configJson,
    NDC_DISABLE_PROJECT_CONFIG: "1",
    NDC_PURE: "1",
    NDC_DISABLE_AUTOUPDATE: "1",
    NDC_DISABLE_AUTOCOMPACT: "1",
    NDC_DISABLE_MODELS_FETCH: "1",
    NDC_AUTH_CONTENT: "{}",
  }
}
...
// Typed equivalent of constructing argv for `ndcode run`. New flags should
...
// `ndcode serve` is a long-lived process — it never exits on its own.
...
// `ndcode acp` speaks newline-delimited JSON-RPC over stdin/stdout. It is
...
export type NeuralDeepCodeCli = {
  // High-level: run a single prompt against the test model. Short-lived.
  readonly run: (message: string, opts?: RunOpts) => Effect.Effect<RunResult>
  // Spawn `ndcode serve` and wait until it's listening. Long-lived: the
  ...
  // Spawn `ndcode acp` and return a duplex JSON-RPC handle. Long-lived:
...
export type CliFixture = {
  ...
  readonly ndcode: NeuralDeepCodeCli
}
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const spawn = Effect.fn("ndcode.spawn")(function* (args: string[], opts?: SpawnOpts) {
    ...
    const serve = Effect.fn("ndcode.serve")(function* (opts?: ServeOpts) {
      const argv = ["serve"]
      ...
      //   "ndcode server listening on http://<host>:<port>"
      ...
      const match = yield* Deferred.await(readyDeferred).pipe(
        Effect.timeoutOrElse({
          ...
          orElse: () =>
            Effect.fail(
              new Error(
                `ndcode serve did not become ready within ${readyTimeoutMs}ms\n` +
    ...
    const acp = Effect.fn("ndcode.acp")(function* (opts?: AcpOpts) {
      const argv = ["acp"]
    ...
    const ndcode: NeuralDeepCodeCli = { run, serve, acp, spawn, expectExit, parseJsonEvents }

    return yield* fn({ llm, home, ndcode })
    // FetchHttpClient is provided so test bodies can `yield* HttpClient.HttpClient`
    // and hit endpoints on `ndcode.serve()` without rolling their own fetch.
  }).pipe(
    Effect.provide(
...
function expectExit(result: RunResult, expected: number, label = "ndcode") {
...
// every call site. Use this for any test that needs the ndcode CLI fixture.
//
...
// resources (e.g. `ndcode.serve`) without an extra `Effect.scoped` wrapper —
