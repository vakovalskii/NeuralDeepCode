...
import { ProjectDirectories } from "@neuraldeepcode/core/project/directories"
...
describe("Project.fromDirectory", () => {
  it.live("should handle git repository with no commits", () =>
    Effect.gen(function* () {
      ...
      const ndcodeFile = path.join(tmp, ".git", "ndcode")
      expect(yield* Effect.promise(() => Bun.file(ndcodeFile).exists())).toBe(false)
    }),
  )
...
describe("Project.fromDirectory with worktrees", () => {
  ...
  it.live("worktree should share project ID with main repo", () =>
    Effect.gen(function* () {
      ...
      const cache = path.join(tmp, ".git", "ndcode")
...
describe("Project.fromDirectory with bare repos", () => {
  it.live("worktree from bare repo should cache in bare repo, not parent", () =>
    Effect.gen(function* () {
      ...
      const correctCache = path.join(barePath, "ndcode")
      const wrongCache = path.join(parentDir, ".git", "ndcode")
  ...
  it.live("different bare repos under same parent should not share project ID", () =>
    Effect.gen(function* () {
      ...
      const cacheA = path.join(bareA, "ndcode")
      const cacheB = path.join(bareB, "ndcode")
      const wrongCache = path.join(parentDir, ".git", "ndcode")
