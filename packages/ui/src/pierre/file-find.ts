...
function clearHighlightFind() {
  ...
  if (!api) return
  api.delete("ndcode-find")
  api.delete("ndcode-find-current")
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const setHighlights = (ranges: Range[], currentIndex: number) => {
    ...
    if (active) api.set("ndcode-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== currentIndex)
    if (rest.length > 0) api.set("ndcode-find", new Highlight(...rest))
    return true
  }
