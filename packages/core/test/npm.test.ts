...
describe("Npm.sanitize", () => {
  test("keeps normal scoped package specs unchanged", () => {
    expect(Npm.sanitize("@neuraldeepcode/acme")).toBe("@neuraldeepcode/acme")
    expect(Npm.sanitize("@neuraldeepcode/acme@1.0.0")).toBe("@neuraldeepcode/acme@1.0.0")
    expect(Npm.sanitize("prettier")).toBe("prettier")
  })

  test("handles git https specs", () => {
    const spec = "acme@git+https://github.com/ndcode/acme.git"
    const expected = win ? "acme@git+https_//github.com/ndcode/acme.git" : spec
