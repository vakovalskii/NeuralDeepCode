...
import { Watcher } from "@neuraldeepcode/core/filesystem/watcher"
...
const flagsLayer = ConfigProvider.layer(
  ConfigProvider.fromUnknown({
    NDC_EXPERIMENTAL_FILEWATCHER: "true",
    NDC_EXPERIMENTAL_DISABLE_FILEWATCHER: "false",
  }),
)
...
function withTmp<A, E, R>(
  ...
  return Effect.acquireRelease(
    Effect.promise(async () => {
      ...
      await $`git config user.email test@ndcode.test`.cwd(tmp.path).quiet()
