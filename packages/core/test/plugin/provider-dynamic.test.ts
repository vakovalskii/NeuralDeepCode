...
import { AISDK } from "@neuraldeepcode/core/aisdk"
...
import { DynamicProviderPlugin } from "@neuraldeepcode/core/plugin/provider/dynamic"
...
function tempEntrypoint(source: string) {
  return Effect.acquireRelease(
    Effect.promise(async () => {
      const directory = await fs.mkdtemp(path.join(os.tmpdir(), "ndcode-provider-dynamic-"))
