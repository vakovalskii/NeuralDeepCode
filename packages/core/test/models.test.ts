...
// test/preload.ts pins NDC_MODELS_PATH to a fixture so other tests can
...
// bun process.
const ORIGINAL_MODELS_PATH = Flag.NDC_MODELS_PATH
const ORIGINAL_DISABLE_FETCH = Flag.NDC_DISABLE_MODELS_FETCH
beforeAll(() => {
  Flag.NDC_MODELS_PATH = undefined
  Flag.NDC_DISABLE_MODELS_FETCH = true
})
afterAll(() => {
  Flag.NDC_MODELS_PATH = ORIGINAL_MODELS_PATH
  Flag.NDC_DISABLE_MODELS_FETCH = ORIGINAL_DISABLE_FETCH
})
  ...
  it.live("get() recovers from a corrupted cache file by fetching a fresh catalog", () =>
    Effect.gen(function* () {
      ...
      const result = yield* Effect.acquireUseRelease(
        Effect.sync(() => {
          Flag.NDC_DISABLE_MODELS_FETCH = false
        }),
        () =>
          provided(
            state,
