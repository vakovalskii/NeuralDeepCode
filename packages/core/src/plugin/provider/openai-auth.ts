...
function headers(contentType: string) {
  return { "Content-Type": contentType, "User-Agent": `ndcode/${InstallationVersion}` }
}
...
const successPage =
  "<!doctype html><title>NeuralDeepCode</title><h1>Authorization successful</h1><p>You can close this window.</p>"
const errorPage = (message: string) =>
  `<!doctype html><title>NeuralDeepCode</title><h1>Authorization failed</h1><p>${message.replace(/[&<>"']/g, "")}</p>`
