...
import { ToolFailure } from "@neuraldeepcode/llm"
...
export class ConfigService extends Context.Service<ConfigService, Config>()("@neuraldeepcode/v2/WebSearchConfig") {}

/** Isolates the retained product environment contract from the generic tool implementation. */
export const defaultConfigLayer = Layer.sync(ConfigService, () =>
  ConfigService.of({
    provider:
      process.env.NDC_WEBSEARCH_PROVIDER === "exa" || process.env.NDC_WEBSEARCH_PROVIDER === "parallel"
        ? process.env.NDC_WEBSEARCH_PROVIDER
        : undefined,
    enableExa: truthy("NDC_EXPERIMENTAL") || truthy("NDC_ENABLE_EXA") || truthy("NDC_EXPERIMENTAL_EXA"),
    enableParallel: truthy("NDC_ENABLE_PARALLEL") || truthy("NDC_EXPERIMENTAL_PARALLEL"),
