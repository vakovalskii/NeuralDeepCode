...
import { createNeuralDeepCode } from "@neuraldeepcode/sdk"
...
`)
  ...
  const ndcode = await createNeuralDeepCode({ port: 0 })
  ...
  try {
    ...
    const session = await ndcode.client.session.create()
    const result = await ndcode.client.session
      .prompt({
